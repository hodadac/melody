...
public class AlbumService {
    ...
    public List<Album> searchAlbumsByTitle(String title) {
        CriteriaBuilder cbAlbum = entityManager.getCriteriaBuilder();
        CriteriaQuery<Album> criteriaQuery = cbAlbum.createQuery(Album.class);
        Root<Album> root = criteriaQuery.from(Album.class);

        // Create a predicate to filter songs by title
        Predicate titlePredicate = cbAlbum.like(
                cbAlbum.lower(root.get("albumTitle")),
                "%" + title.toLowerCase() + "%"
        );
